...
namespace IDS.Paderborn.FreeCATMA5.Process
{
  public static class V2
  {
    public static void Process(string dir)
    {
      ...
      var output = Path.Combine(dir, "output_v2");
      ...
      File.WriteAllText(dir + $"_v2_{(valid ? "valid" : "error")}.json", JsonConvert.SerializeObject(new Document
      {
    ...
    private static void BodyLoopSeg(HtmlDocument xml, HtmlNode htmlNode, ref StringBuilder text, ref List<Annotation> annoValues)
    {
      ...
      foreach (var x in anas)
      {
        ...
        valueChain.Add(valNode.ChildNodes.FirstOrDefault(x => x.Name == "fsdescr")?.InnerText);

        for (var i = 0; i < valueChain.Count; i++)
        {
